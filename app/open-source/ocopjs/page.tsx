    ...
    <Fragment>
      <article className="space-y-6 mt-8">
        ...
        <section className="mx-auto max-w-screen-sm px-6">
          ...
          <p>
            @ocopjs cung cấp đủ các kiểu dữ liệu gốc và một số dạng tuỳ biến với
            mục đích dễ sử dụng như: colors, calendar,...
          </p>
          <p>
            Từ thiết kế cơ sở dữ liệu của bạn, cấu hình cho @ocopjs giúp bạn dễ
            dàng phát triển nhờ có sẵn API CRUD cơ bản và các hàm để viết sự
            kiện (hooks) cũng như kiểm soát truy cập (access control)
          </p>

          <h3>2. Nhập liệu nội dung</h3>
          <p>
            Giao diện quản trị mặc định của @ocopjs dựng đủ các trang nhập liệu
            cần thiết. Hơn nữa còn cung cấp các trang danh sách với khả năng lọc
            kết quả mạnh mẽ.
          </p>
          ...
          <p>
            Bạn có thể áp dụng các phương án triển khai tự động bằng docker lên
            các môi trường chuyên dụng như Heroku, Vercel, Netlify, AWS, Azure,
            Google Cloud,... Ngoài ra, với chi phí thấp, @ocopjs cung cấp một số
            thư viện giúp bạn có thể tự triển khai trên các máy chủ của mình
            hoàn toàn tự động.
          </p>
        </section>
